  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        <StatCard
          label="真人访客"
          value={String(summary.total_human_uv)}
          detail={`另有单次来源 ${summary.total_single_uv} 个无法判定`}
          icon={Eye}
          color="text-indigo-600 bg-indigo-50"
        />
        <StatCard
          label="真人页面浏览"
          value={String(summary.total_human_pv)}
          detail={`原宽口径 ${summary.total_pv}（含页面带出的接口请求）`}
        ...
        <StatCard
          ...
          detail={`${summary.total_registrations} 注册 / ${summary.total_uv} 来源`}
          icon={Percent}
        ...
        <StatCard
          ...
          detail={`另剔除伪装脚本来源 ${summary.total_machine_uv} 个`}
          icon={ShieldBan}
      ...
      <div className="rounded-2xl border border-paper-300 bg-white p-5">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="text-xs text-ink-400">
            最新一天（{last?.date}）：真人 {last?.human_uv} · 页面 {last?.human_pv} · 注册 {last?.registrations}
          </span>
        </div>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 5, right: 10, bottom: 0, left: -10 }}>
              ...
              <Tooltip />
              <Legend />
              <Bar yAxisId="left" dataKey="真人页面浏览" fill="#10b981" radius={[3, 3, 0, 0]} />
              <Bar yAxisId="left" dataKey="真人访客" fill="#6366f1" radius={[3, 3, 0, 0]} />
              <Line
                yAxisId="right"
                type="monotone"
                dataKey="转化率"
        ...
        <table className="w-full text-sm">
          <thead className="bg-paper-50 text-left text-xs text-ink-500">
            <tr>
              ...
              <th className="px-4 py-2.5 font-medium">真人 UV</th>
              <th className="px-4 py-2.5 font-medium">真人页面浏览</th>
              <th className="px-4 py-2.5 font-medium">单次来源</th>
              <th className="px-4 py-2.5 font-medium">剔除脚本</th>
          ...
          <tbody>
            {[...days].reverse().map((d) => (
              <tr key={d.date} className="border-t border-paper-200">
                ...
                <td className="px-4 py-2.5">{d.human_uv}</td>
                <td className="px-4 py-2.5">{d.human_pv}</td>
                <td className="px-4 py-2.5 text-ink-400">{d.single_uv}</td>
                <td className="px-4 py-2.5 text-ink-400">{d.machine_uv}</td>
