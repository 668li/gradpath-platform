...
class Mentor(UUIDMixin, TimestampMixin, Base):
    ...
    research_directions: Mapped[list] = mapped_column(
    ...
    academic_homepage: Mapped[str | None] = mapped_column(
    ...
    enrollment_directions: Mapped[list] = mapped_column(
        JSONB, nullable=False, default=list
    )  # 当前招生方向
    ...
    rating_relationship: Mapped[float] = mapped_column(
    ...
    source_platform: Mapped[str] = mapped_column(
        String(100), nullable=False, default="official"
    )  # official/crawler/user
