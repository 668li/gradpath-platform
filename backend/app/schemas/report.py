...
class ReportCreateRequest(BaseModel):
    ...
    reason: str = Field(min_length=1, max_length=100, description="举报原因（如：广告/人身攻击/不实信息）")
...
class ReportProcessRequest(BaseModel):
    ...
    ban_author: bool = Field(default=False, description="action=processed 时是否同时封禁被举报对象作者")
    ban_reason: str | None = Field(default=None, max_length=500, description="封禁原因（ban_author=true 必填）")
