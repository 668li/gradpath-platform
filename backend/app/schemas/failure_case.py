...
class FailureCaseCreate(BaseModel):
    ...
    author_role: str = Field(
        ..., min_length=1, max_length=50, description="作者身份，如 在校生/毕业生/工作3年内"
    )
    path_type: str = Field(
        ..., max_length=50, description="路径：kaoyan/civil_service/employment/study_abroad"
    )
    stage: str = Field(
        ..., max_length=50, description="阶段：preparation/interview/final_year1/year2_plus"
    )
    ...
    what_would_i_do: str = Field(
        ..., min_length=1, max_length=10000, description="如果重来会怎么做"
    )
