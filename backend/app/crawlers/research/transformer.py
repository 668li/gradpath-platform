...
# 明确离题"强锚点"黑名单（游戏 / 娱乐 / 与职业提升无关的专有名词）。
# 只有"几乎不可能出现在正当考研/考公/考证/就业内容里"的多字专有名词才能进这组，
# 命中任一即判离题（优先级最高，覆盖"心态/压力"等通用情绪词）。
# 禁止放通用词（匹配/上分/段位/猫/狗/乐观/经验/段子/美食/端游/追剧等）——
# 它们会误伤"唐迟考研英语新题型(匹配)"、"华中农大小猫学姐"等正当内容，见 09-02 误判事故。
OFF_TOPIC_REJECT_KEYWORDS = [
    # 游戏专有名词 / 电竞赛事
    "三角洲", "三角洲行动", "原神", "王者荣耀", "和平精英", "英雄联盟", "LOL", "绝地求生", "PUBG",
    "CSGO", "无畏契约", "瓦罗兰特", "崩坏", "星穹铁道", "明日方舟", "阴阳师", "元气骑士",
    "我的世界", "switch", "ps5", "steam", "GTA", "塞尔达",
    "LPL", "KPL", "世冠", "季后赛", "排位赛", "游戏开黑", "开黑",
    # 动漫专有名词（几乎不会出现在考研/考公内容里）
    "火影", "火影忍者", "海贼王", "斗罗大陆", "龙珠", "死神",
    # 娱乐八卦专有名词
    "娱乐圈", "绯闻", "选秀", "吃瓜", "明星同款", "演唱会",
    # 玄学占卜（非职业提升）
    "塔罗", "占卜", "星座运势",
    # 其他明显非职业提升的专有名词
    "约p",
    # 体育赛事（固有名/赛事名，不含泛化"体育"）——含"考研大纲解析"等仍需靠领域词放行
    "NBA", "世界杯", "欧冠", "英超", "西甲", "CBA",
]
...
def classify_topic_relevance(
    ...
    Returns:
        ...
        - is_off_topic：True=明确离题（命中强锚点）；False=相关（命中领域词）；
          None=无强离题信号也无领域词（存疑，放行交人工审核，不在 feed 自动隐藏）。
        - reason：判定说明；True 时给出命中强锚点词，None 时给出"无领域信号待人工确认"
    ...
    判定策略（09-02 修正：强锚点→领域词→存疑放行，杜绝通用词误伤）：
        ...
        2) 命中 PLATFORM_DOMAIN_KEYWORDS → 相关（返回归属领域）。
           注意：通用词（匹配/猫/狗/段子/美食等）不在黑名单，若正文同时含考研领域词，
           领域词优先，保证"唐迟考研英语新题型(含『匹配』)"等正当内容不被误伤。
        3) 均未命中 → 返回 None（存疑，不自动隐藏）。这避免"汤家凤/李永乐概率论"这类
           无关键词但明显服务考研人群的内容被误杀——由管理员/feed 审核决定去留。
    """
    ...
    # 1) 强离题锚点优先：命中即离题（强信号，优先级最高，可覆盖领域词）。
    #    OFF_TOPIC_REJECT_KEYWORDS 只含"多字专有名词"，不含通用词——
    #    通用词若误入会把这层的强语义冲淡，见 09-02 匹配/猫/狗误判事故。
    for kw in OFF_TOPIC_REJECT_KEYWORDS:
        if kw.lower() in text:
            return True, f"命中离题词「{kw}」", None

    # 2) 领域词：判定相关并标注归属领域。命中任一领域词即相关。
    for kw in PLATFORM_DOMAIN_KEYWORDS:
        if kw.lower() in text:
            domain = _infer_domain(title_s, content_s)
            return False, "", domain

    # 3) 无强离题锚点也无领域词 → 存疑放行（不自动隐藏，交人工审核）。
    #    早期把这里判为"离题"误杀了汤家凤/李永乐等无关键词的正当内容。
    return None, "无领域信号待人工确认", None
