...
async def scrape_zhipin(page, keyword):
    ...
    try:
        ...
        for card in cards[:20]:
            try:
                title_el = await card.query_selector(".job-name") or await card.query_selector(
                    "[class*='job-name']"
                )
                company_el = await card.query_selector(
                    ".company-name a"
                ) or await card.query_selector("[class*='company-name']")
                salary_el = await card.query_selector(".salary") or await card.query_selector(
                    "[class*='salary']"
                )
                area_el = await card.query_selector(".job-area") or await card.query_selector(
                    "[class*='job-area']"
                )
...
async def scrape_liepin(page, keyword):
    ...
    try:
        await page.goto(url, wait_until="domcontentloaded", timeout=30000)
        await page.wait_for_timeout(3000)
        cards = await page.query_selector_all(".job-card") or await page.query_selector_all(
            "[class*='job-card']"
        )
        if not cards:
        ...
        for card in cards[:20]:
            try:
                title_el = await card.query_selector(
                    "[class*='job-title'] a"
                ) or await card.query_selector("[class*='job-title']")
                company_el = await card.query_selector(
                ...
                area_el = await card.query_selector(
                    "[class*='job-dq']"
                ) or await card.query_selector("[class*='job-area']")
