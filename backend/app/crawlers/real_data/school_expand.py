            ...
            schools.append(
                {
                    ...
                    "city": "",
                    "level": (
                        "985" if name in real_985 else ("211" if name in real_211 else "双一流")
                    ),
                    "type": "",
                }
            )
...
def import_schools(db, schools):
    """导入院校数据到 schools 表"""
    # 获取已有校名去重
    existing_names = set(row[0] for row in db.query(School.name).all())
...
def main():
    ...
    try:
        ...
        rows = db.execute(
            text(
                "SELECT province, COUNT(*) FROM schools WHERE province IS NOT NULL AND province != '' GROUP BY province ORDER BY COUNT(*) DESC LIMIT 10"
            )
        ).fetchall()
        for row in rows:
            print(f"  {row[0]}: {row[1]}")
