...
with sync_playwright() as p:
    ...
    for kw in keywords:
        ...
        try:
            page.goto(
                f"https://search.bilibili.com/all?keyword={kw}",
            ...
            for link in links:
                ...
                if bvid and bvid.group() not in seen and text and len(text) > 3:
                    seen.add(bvid.group())
                    results.append(
                        {"title": text[:200], "url": href, "bvid": bvid.group(), "keyword": kw}
                    )
...
with open(
    r"D:\职业规划\职业规划\backend\app\crawlers\real_data\bilibili_round3.json",
    "w",
    encoding="utf-8",
) as f:
