    ...
    for kw in API_KEYWORDS:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.code == 412:
                    wait = (attempt + 1) * 5
                    print(f"  412 限流，等待 {wait}s 后重试 ({attempt+1}/{max_retries})")
                    time.sleep(wait)
                    # 重新预热首页
                    try:
                        req2 = urllib.request.Request("https://www.bilibili.com", headers=HEADERS)
                        urllib.request.urlopen(req2, timeout=10).read()
                    except Exception:
                        pass
                else:
                    print(f"  [API] 搜索 {kw} HTTP 错误: {e}")
                    break
            except Exception as e:
                print(f"  [API] 搜索 {kw} 失败: {e}")
                break

        time.sleep(2)  # 关键词之间更长的延迟
