...
cmd = ["docker", "exec", "gradpath-backend-1", "bash", "-c", f"cat {parts} > {DEST}"]
r = subprocess.run(cmd, capture_output=True, text=True)
if r.returncode != 0:
...
# Cleanup
cmd = ["docker", "exec", "gradpath-backend-1", "bash", "-c", "rm -f /tmp/kd_*.bin"]
subprocess.run(cmd, capture_output=True)

# Verify
cmd = ["docker", "exec", "gradpath-backend-1", "bash", "-c", f"wc -c < {DEST}"]
r = subprocess.run(cmd, capture_output=True, text=True)
