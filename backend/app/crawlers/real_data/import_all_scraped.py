    ...
    try:
        ...
        for page in crawled:
            if not isinstance(page, dict):
                continue
            title = safe_str(page.get("title", "")) or safe_str(
            ...
            for section, pages in yz_data.get("data", {}).items():
                ...
                for page in pages:
                    ...
                    existing = (
                        ...
                        .filter(KnowledgeArticle.title == url[:200])
                        .first()
                    )
                    if existing:
                        total_skipped += 1
        ...
        for vid in bili:
            ...
            if not title:
                continue
            existing = (
                db.query(KnowledgeArticle).filter(KnowledgeArticle.title == title[:200]).first()
            )
            if existing:
                total_skipped += 1
                continue
        ...
        if isinstance(yz2, dict):
            for section_name, section_data in yz2.get("sections", {}).items():
                ...
                for page in pages:
                    if not isinstance(page, dict):
                        continue
                    content = safe_str(page.get("markdown", "")) or safe_str(
                        page.get("content", "")
                    )
                    if not content or len(content) < 100:
                        continue
        ...
        if isinstance(kool, dict) and "data" in kool:
            for page in kool["data"]:
                ...
                if existing:
                    ...
                    continue
                existing_ka = (
                    db.query(KnowledgeArticle).filter(KnowledgeArticle.title == title).first()
                )
                if existing_ka:
                    total_skipped += 1
                    continue
        ...
        for col in colleges:
            ...
            level = (
                "985"
                if "985高校" in tags
                else ("211" if "211高校" in tags else ("双一流" if "双一流" in tags else "普通"))
            )
            db.add(
                School(
                    name=name[:100],
                    slug=slug,
                    province=province[:20] if province else None,
                    level=level,
        ...
        try:
            r = db.execute(
                text("SELECT COUNT(*) FROM experience_posts WHERE source_platform='crawler'")
            )
            print(f"\n  爬取经验帖: {r.scalar()}")
        except:
            pass
        ...
        try:
            r = db.execute(
                text(
                    "SELECT COUNT(*) FROM knowledge_articles WHERE source IS NOT NULL AND source != ''"
                )
            )
