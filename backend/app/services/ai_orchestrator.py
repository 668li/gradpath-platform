...
"""统一 LLM 入口 — 编排 AIService 与熔断器。

行为与原 ``AIService.chat`` 等价（未配置时抛 ``AIServiceNotConfigured``），
叠加 B8 熔断器（连续 5 次失败打开熔断，30s 后半开试探）避免雪崩。
"""
...
class AIOrchestrator:
    ...
    - 行为与 ``AIService.chat`` 等价，仅叠加 B8 熔断器保护
      （连续 5 次失败打开熔断，30s 后半开试探）。
    - ``AIService.chat`` 内部已用 tenacity 重试超时/网络错误，
      本层不再叠加第二层重试或缓存（2026-09-25 ponytail 瘦身：
      use_cache 无调用方、retry 循环与内部重试等价，均已删）。
    """
    ...
    async def chat(
        ...
        """
        # 熔断器打开时直接抛 AICircuitBreakerOpenError，不发送实际 LLM 请求
