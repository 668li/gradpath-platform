...
def calculate_xp(db: Session, user_id: UUID) -> int:
    ...
    events_stats = (
        db.query(
            ...
            func.count(
                case(
                    (
                        CareerEvent.event_type.in_([EventType.promotion, EventType.certification]),
                        1,
                    ),
                    else_=None,
                )
    ...
    skills_level_sum = (
        db.query(func.sum(SkillNode.level)).filter(SkillNode.user_id == user_id).scalar() or 0
    )
    retros = (
        db.query(func.count(Retrospective.id)).filter(Retrospective.user_id == user_id).scalar()
        or 0
    )
    community = (
        db.query(func.count(CommunityReport.id)).filter(CommunityReport.user_id == user_id).scalar()
        or 0
    )
    interviews = (
        db.query(func.count(InterviewReport.id)).filter(InterviewReport.user_id == user_id).scalar()
        or 0
    )
...
BADGE_REGISTRY: list[dict] = [
    ...
    },
    {
        ...
        "name": "经验分享",
        "description": "提交第一份面试经验",
        "icon": "briefcase",
        "check": lambda ctx: ctx.interview_count >= 1,
    },
    {
        "code": "decision_master",
        "name": "决策大师",
        "description": "创建 5 个以上去向决策",
        "icon": "compass",
    ...
    },
    {
        "code": "retro_master",
        "name": "复盘行者",
        "description": "完成 5 次以上复盘",
        "icon": "clipboard",
        "check": lambda ctx: ctx.retros_count >= 5,
    },
    {
        "code": "community_master",
        "name": "社区先锋",
        "description": "提交 3 份以上社区报告",
        "icon": "users",
...
def check_and_award_badges(db: Session, user_id: UUID) -> tuple[list[dict], int]:
    ...
    existing_codes = {
        ub.badge_code for ub in db.query(UserBadge).filter(UserBadge.user_id == user_id).all()
    }
...
def get_profile(db: Session, user_id: UUID) -> dict:
    ...
    progress_percent = (
        round(progress_current / progress_needed * 100) if progress_needed > 0 else 100
    )
