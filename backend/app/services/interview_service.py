...
from app.schemas.interview import InterviewAggregateResponse, InterviewStats, InterviewSubmit
...
def list_my_reports_paginated(
    ...
    """分页查询当前用户提交的面试报告（按面试年份降序）。"""
    query = db.query(InterviewReport).filter(InterviewReport.user_id == user_id)
    total = query.count()
    items = (
        query.order_by(InterviewReport.interview_year.desc())
...
def aggregate(db: Session, company: str, position: str | None = None) -> InterviewAggregateResponse:
    ...
    if position:
        filters.append(InterviewReport.position.ilike(f"%{escape_like(position)}%", escape="\\"))

    sample_count = db.query(func.count(InterviewReport.id)).filter(*filters).scalar() or 0
    ...
    result_distribution = {r.value: count / sample_count for r, count in result_rows}

    # 考察维度频率：由于 dimensions 存在 JSONB 数组中，
    # 需要逐行统计（SQLite 测试环境不支持 JSONB 查询）
    all_reports = db.query(InterviewReport.dimensions).filter(*filters).all()
    dim_count: dict[str, int] = {}
    for (dims,) in all_reports:
        if dims:
            for d in dims:
                dim_count[d] = dim_count.get(d, 0) + 1
    dimension_frequency = {d: count / sample_count for d, count in dim_count.items()}
    ...
    if not position:
        ...
        common_positions = [{"name": name, "count": count} for name, count in pos_rows]
...
def list_companies(db: Session, keyword: str = "") -> list[dict]:
    ...
    filters = []
    if keyword:
        filters.append(InterviewReport.company.ilike(f"%{escape_like(keyword)}%", escape="\\"))
    rows = (
        db.query(
