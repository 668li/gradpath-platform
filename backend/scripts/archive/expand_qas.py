...
with engine.connect() as conn:
    ...
    for _ in range(100000):
        ...
        conn.execute(
            text(
                "INSERT INTO qas (id, user_id, title, content, tags, status, view_count, answer_count, is_resolved, created_at, updated_at) VALUES (gen_random_uuid(), :uid, :t, :c, :tags, :st, :vc, :ac, :ir, NOW(), NOW())"
            ),
            {
                ...
                "vc": random.randint(10, 5000),
                "ac": random.randint(0, 10),
                "ir": random.choice([True, False]),
            },
        )
        count += 1
    conn.commit()
