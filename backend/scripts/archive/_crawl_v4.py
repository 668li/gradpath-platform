    ...
    for kw in keywords:
        try:
            ...
            for v in results[:25]:
                ...
                cur.execute(
                    "INSERT INTO knowledge_articles (title, content, source, category, tags, metadata, is_published, created_at) SELECT %s, %s, 'bilibili', %s, %s::jsonb, '{}'::jsonb, true, NOW() WHERE NOT EXISTS (SELECT 1 FROM knowledge_articles WHERE title=%s)",
                    (title, content, kw, json.dumps([kw]), title)
                )
                total += cur.rowcount
            conn.commit()
            print(f"  [{kw}] {len(results)}条 → +{cur.rowcount}条 (累计{total})")
            time.sleep(2)
    ...
    for kw in keywords:
        try:
            ...
            for repo in repos:
                ...
                content = f"{desc}\n\nStars: {stars}\n{html_url}"
                cur.execute(
                    "INSERT INTO knowledge_articles (title, content, source, category, tags, metadata, is_published, created_at) SELECT %s, %s, 'github', %s, %s::jsonb, '{}'::jsonb, true, NOW() WHERE NOT EXISTS (SELECT 1 FROM knowledge_articles WHERE title=%s)",
...
def crawl_huatu(conn):
    ...
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as c:
            ...
            # 提取页面中所有文本块作为职位信息
            for tag in soup.select('li, h2, h3, .item, .list-item, a'):
                ...
                cur.execute(
                    ...
                    (text, f"来源: 华图教育\n{href}" if href else f"来源: 华图教育", json.dumps(["华图教育"]), json.dumps(["国考"]), text)
                )
                total += cur.rowcount
            conn.commit()
            print(f"  华图: +{total}条")
    ...
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as c:
            ...
            for a in soup.select('a'):
                ...
                if not any(kw in text for kw in ["考研", "专业", "院校", "分数", "调剂", "复试", "初试"]): continue
    ...
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as c:
            ...
            for a in soup.select('a'):
                ...
                cur.execute(
                    "INSERT INTO grad_adjustment_info (university_name, department, major_name, source_url, year, status, data_sources, created_at) SELECT %s, '待查', '待查', %s, 2026, 'active', %s, NOW() WHERE NOT EXISTS (SELECT 1 FROM grad_adjustment_info WHERE university_name=%s)",
                    (text, href, json.dumps(["研招网"]), text)
                )
                total += cur.rowcount
            conn.commit()
            print(f"  研招网: +{total}条")
...
if __name__ == "__main__":
    print(f"GradPath 综合爬虫 v4 — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
