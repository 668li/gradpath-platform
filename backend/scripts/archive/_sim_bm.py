...
# 选2篇时间管理 + 1篇记忆科学
cur.execute(
    "SELECT id FROM knowledge_articles WHERE category='学习方法' AND tags::text LIKE '%时间管理%' LIMIT 2"
)
tm = [r[0] for r in cur.fetchall()]
cur.execute(
    "SELECT id FROM knowledge_articles WHERE category='学习方法' AND tags::text LIKE '%记忆科学%' LIMIT 1"
)
