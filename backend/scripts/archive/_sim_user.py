...
cur.execute(
    "SELECT id, tags FROM knowledge_articles WHERE category='学习方法' AND tags::text LIKE '%时间管理%' LIMIT 2"
)
tm_articles = cur.fetchall()
cur.execute(
    "SELECT id, tags FROM knowledge_articles WHERE category='学习方法' AND tags::text LIKE '%记忆科学%' LIMIT 1"
)
...
for i, (aid, tags) in enumerate(all_selected):
    ...
    bid = str(uuid.uuid4())
    cur.execute(
        "INSERT INTO bookmarks (id, user_id, target_type, target_id, created_at) VALUES (%s, %s, 'post', %s, %s)",
        (bid, uid, str(aid), created),
    )

# 模拟评估数据
cur.execute(
    "INSERT INTO assessments (id, user_id, result_code, recommended_directions, created_at) VALUES (%s, %s, 'RIA', %s, NOW())",
