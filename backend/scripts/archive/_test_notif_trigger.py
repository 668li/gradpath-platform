...
cur = conn.cursor()
cur.execute(
    "SELECT id, user_id, title FROM experience_posts WHERE user_id != (SELECT id FROM users WHERE email='test2@example.com') LIMIT 1"
)
row = cur.fetchone()
cur.close()
conn.close()
...
c = req(
    "POST",
    "/api/comments",
    {"post_id": str(pid), "content": "测试触发通知的评论", "parent_id": None},
    token,
)
...
if owner_token:
    ...
    print(
        "Owner unread notifications:", n[0].get("unread_count", 0) if isinstance(n[0], dict) else n
    )
else:
