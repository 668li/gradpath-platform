...
schools = conn.execute(
    text("SELECT DISTINCT school_name FROM grad_school_intel ORDER BY school_name LIMIT 30")
).fetchall()
print("Schools sample:", [s[0] for s in schools])
majors = conn.execute(
    text("SELECT DISTINCT major_name FROM grad_school_intel ORDER BY major_name")
).fetchall()
