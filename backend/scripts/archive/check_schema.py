...
with engine.connect() as conn:
    cols = conn.execute(
        text(
            "SELECT column_name FROM information_schema.columns WHERE table_name = 'qas' ORDER BY ordinal_position"
        )
    ).fetchall()
    print("qas columns:", [c[0] for c in cols])
    cols2 = conn.execute(
        text(
            "SELECT column_name FROM information_schema.columns WHERE table_name = 'experience_posts' ORDER BY ordinal_position"
        )
    ).fetchall()
