...
with engine.connect() as conn:
    ...
    try:
        ...
        result = conn.execute(
            text(
                "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'qa_answers' ORDER BY ordinal_position"
            )
        )
    ...
    # Sample qas
    result = conn.execute(
        text("SELECT id, title, best_answer_id FROM qas WHERE best_answer_id IS NULL LIMIT 5")
    )
