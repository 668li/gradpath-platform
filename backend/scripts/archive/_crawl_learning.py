...
def classify_tags(title, content):
    ...
    tags = []
    if any(k in text for k in ["记忆", "遗忘", "艾宾浩斯", "间隔", "重复"]):
        tags.append("记忆科学")
    if any(k in text for k in ["番茄", "时间管理", "专注", "碎片时间", "日程"]):
        tags.append("时间管理")
    if any(k in text for k in ["费曼", "主动学习", "输出", "教别人", "实践"]):
        tags.append("学习策略")
    if any(k in text for k in ["习惯", "21天", "行为", "坚持", "打卡"]):
        tags.append("习惯养成")
    if any(k in text for k in ["思维导图", "笔记", "卡片", "手写", "整理"]):
        tags.append("笔记方法")
    if any(k in text for k in ["认知", "元认知", "注意力", "大脑", "神经"]):
        tags.append("认知科学")
    if any(k in text for k in ["睡眠", "运动", "饮食", "心态", "焦虑"]):
        tags.append("身心调节")
    if any(k in text for k in ["考试", "备考", "复习", "真题", "冲刺"]):
        tags.append("备考策略")
            ...
            for repo in repos:
                ...
                cur.execute(
                    ...
                    (
                        ...
                        content,
                        json.dumps(tags),
                        json.dumps({"source_url": html_url, "stars": stars}),
                    ),
                )
                count += cur.rowcount
            conn.commit()
            ...
            for a in results[:10]:
                ...
                cur.execute(
                    ...
                    (
                        ...
                        f"来源: DuckDuckGo搜索\n链接: {href}",
                        json.dumps(tags),
                        json.dumps({"source_url": href, "query": q}),
                    ),
                )
                count += cur.rowcount
            conn.commit()
