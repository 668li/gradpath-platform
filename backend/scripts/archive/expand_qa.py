...
with engine.connect() as conn:
    ...
    for (qa_id,) in qas:
        ...
        conn.execute(
            text(
                "INSERT INTO qa_answers (id, qa_id, content, user_id, is_best, like_count, status, created_at, updated_at) VALUES (:id, :qid, :c, :uid, false, :lc, 'approved', NOW(), NOW())"
            ),
            {
                "id": str(uuid.uuid4()),
                "qid": qa_id,
