...
# 数据源配置
GRAD_SOURCES = [
    ("研招网", "https://yz.chsi.com.cn/kyzx/kydt/"),
    ("考研帮", "https://www.kaoyan.com/experience/"),
    ("EOL考研", "https://kaoyan.eol.cn/"),
]
CIVIL_SOURCES = [
    ("中公教育", "https://www.offcn.com/gwy/"),
    ("华图教育", "https://www.huatu.com/guojia/"),
    ("环球网校", "https://www.hqwx.com/gjgwy-kaoshi/"),
]
CAREER_SOURCES = [("前程无忧", "https://www.51job.com/"), ("BOSS直聘", "https://www.zhipin.com/")]
NEWS_SOURCES = [
    ("新浪教育", "https://edu.sina.com.cn/"),
    ("新华网教育", "http://www.xinhuanet.com/edu/"),
    ("网易教育", "https://edu.163.com/"),
    ("中国教育在线", "https://www.eol.cn/kaoyan/"),
]
...
def crawl_civil(conn):
    ...
    for name, url in CIVIL_SOURCES:
        ...
        for a in soup.select("a[href]"):
            ...
            if not any(
                kw in text
                for kw in ["国考", "省考", "公务员", "行测", "申论", "面试", "招录", "事业单位"]
            ):
                continue
            if not href.startswith("http"):
                href = url.rstrip("/") + "/" + href.lstrip("/")
...
def crawl_career(conn):
    ...
    for name, url in CAREER_SOURCES:
        ...
        for a in soup.select("a[href]"):
            ...
            if not any(
                kw in text
                for kw in ["招聘", "职位", "薪资", "面试", "求职", "offer", "简历", "工作"]
            ):
                continue
            if not href.startswith("http"):
                href = url.rstrip("/") + "/" + href.lstrip("/")
