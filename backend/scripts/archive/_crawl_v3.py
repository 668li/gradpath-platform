    ...
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as c:
            ...
            for a in soup.select("a"):
                ...
                if not any(
                    kw in text
                    for kw in ["国考", "职位", "公告", "公务员", "行测", "申论", "报名", "面试"]
                ):
                    continue
    ...
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as c:
            ...
            for a in soup.select("a"):
                ...
                if len(text) < 6:
                    continue
                if not any(kw in text for kw in ["考研", "专业", "院校", "分数", "调剂", "复试"]):
                    continue
                ...
                cur.execute(
                    ...
                    (
                        cur.execute("SELECT id FROM users LIMIT 1") or cur.fetchone()[0],
                        text,
                        f"来源: 考研帮\n{href}",
                        json.dumps(["考研帮", "考研"]),
                        text,
    ...
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as c:
            ...
            for a in soup.select("a"):
                ...
                if len(text) < 6:
                    continue
                if not any(kw in text for kw in ["大学", "学院", "调剂", "招生"]):
                    continue
