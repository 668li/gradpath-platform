...
with engine.connect() as conn:
    existing_unis = [
        r[0]
        for r in conn.execute(text("SELECT DISTINCT school_name FROM grad_school_intel")).fetchall()
    ]
    existing_majors = [
        r[0]
        for r in conn.execute(text("SELECT DISTINCT major_name FROM grad_school_intel")).fetchall()
    ]
...
with engine.connect() as conn:
    rows = conn.execute(
        text("SELECT school_name, major_name, year FROM grad_school_intel")
    ).fetchall()
    for r in rows:
