...
with engine.connect() as conn:
    ...
    for _ in range(7000):
        ...
        conn.execute(
            text(
                "INSERT INTO companies (id, name, industry, size, stage, headquarters, description, created_at, updated_at) VALUES (:id, :name, :ind, :size, :stage, :hq, :desc, NOW(), NOW())"
            ),
            {
                "id": str(uuid.uuid4()),
                "name": name,
                ...
                "hq": random.choice(cities),
                "desc": f"{name}是一家{random.choice(industries)}领域的企业",
            },
        )
        count += 1
    conn.commit()
