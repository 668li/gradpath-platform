    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            resp = requests.post(
                ...
                json={"email": "test@example.com", "password": "test123456", "name": "测试用户"},
                timeout=3,
            )
            duration = time.time() - start
            ...
            if resp.status_code in [200, 201]:
                ...
                log_result(
                    "认证", "/auth/register", "FAIL", duration, f"Status: {resp.status_code}"
                )
                return None
    except Exception as e:
...
def test_module_with_auth(
    module: str, endpoint: str, method: str = "GET", data: Dict = None, token: str = None
):
    """测试需要认证的模块"""
    try:
        ...
        if method == "GET":
            ...
            resp = requests.post(
                f"{API_URL}{endpoint}", json=data or {}, headers=headers, timeout=3
            )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if "total" in data and "items" in data:
                log_result(
                    module, f"{endpoint} (分页)", "PASS", duration, f"Total: {data['total']}"
                )
                return True
            else:
                log_result(
                    module, f"{endpoint} (分页)", "FAIL", duration, "Missing pagination fields"
                )
                return False
        else:
            log_result(
                module, f"{endpoint} (分页)", "FAIL", duration, f"Status: {resp.status_code}"
            )
            return False
    except Exception as e:
