...
用法（仓库根或 backend/ 目录下）:
    RESET_PASSWORD='<新密码>' python backend/scripts/reset_pass.py user@example.com [user2@example.com ...]
...
# 以 backend/ 目录为基准加入 sys.path，兼容本地与容器内运行（不硬编码 /app）
...
def main() -> int:
    ...
    if not emails:
        print("用法: RESET_PASSWORD='<新密码>' python backend/scripts/reset_pass.py user@example.com [...]")
        return 2
    if not new_password:
