...
with engine.connect() as conn:
    ...
    short_exp = conn.execute(
        text(
            "SELECT COUNT(*) FROM experience_posts WHERE LENGTH(content) < 200 AND content IS NOT NULL"
        )
    ).scalar()
    ...
    with_best = conn.execute(
        text("SELECT COUNT(*) FROM qas WHERE best_answer_id IS NOT NULL")
    ).scalar()
    without_best = conn.execute(
        text("SELECT COUNT(*) FROM qas WHERE best_answer_id IS NULL")
    ).scalar()
