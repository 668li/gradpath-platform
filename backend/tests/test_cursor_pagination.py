...
class TestCursorPaginationUtils:
    ...
    def test_apply_cursor_filter_descending(self, db_session):
        ...
        cursor_post = posts[0]
        cursor = encode_cursor(cursor_post.created_at.isoformat(), str(cursor_post.id))
        query = db_session.query(Post)
        filtered = apply_cursor_filter(
            query,
            cursor,
            time_col=Post.created_at,
            id_col=Post.id,
            descending=True,
        )
...
class TestPostsPublicCursor:
    ...
    def test_topic_type_filter(self, auth_headers, client):
        ...
        resp = client.get("/api/posts/public/cursor?page_size=20&topic_type=school_major")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_invalid_cursor_does_not_crash(self, client):
        ...
        resp = client.get("/api/posts/public/cursor?page_size=10&cursor=not-a-valid-cursor")
        # 实现策略：无效 cursor 被忽略，按第一页返回
        assert resp.status_code == 200

    def test_invalid_topic_type_returns_422(self, client):
        """无效 topic_type 应返回 422。"""
        resp = client.get("/api/posts/public/cursor?page_size=10&topic_type=invalid_type")
...
class TestSchoolsCursor:
    ...
    def test_pagination(self, client, db_session):
        ...
        page2 = client.get(
            "/api/employment/schools/cursor?page_size=1&cursor=" + page1["next_cursor"]
        ).json()
        assert len(page2["items"]) == 1
        assert page2["has_more"] is False
    ...
    def test_invalid_cursor_does_not_crash(self, client, db_session):
        """无效 cursor 不应导致 500 错误。"""
        self._seed_schools(db_session)
        resp = client.get("/api/employment/schools/cursor?page_size=10&cursor=invalid")
        assert resp.status_code == 200
