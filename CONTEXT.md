...
## 信息差域（2026-09-26 正名，用户终审）

### 官方信息差（Official Info Gap）
官方**已发布**但散落难聚合的信息（分数线、招生目录、时间节点、公示名单）。
- 性质：基建营养——喂决策引擎/时间线，不作为对用户宣传的"信息差"卖点。
- 供给：爬虫管道（爬取跟随决策 doctrine）。

### 门道信息差（Insider Info Gap）
官方**永不发布**、散落在 UGC 与圈内的知识：门道潜规则（复试前要不要联系导师、邮件怎么写）、圈内消息（悄悄扩招/改考科目/导师今年招人）、内幕实锤（压分/复试黑幕/一志愿被刷）。
- 性质：产品差异化的真正落点；竞品无系统化带源供给。
- 供给：AI 定向搜集+交叉验证→门道卡（≥2 独立来源转正，孤证明标）；长线补用户回传。
- 与官方信息差的关系：门道卡里的"实锤"类主张可引用官方公示数据（复试差额比等）作客观证据层。

### 门道卡（Intel Card）
一条结构化的门道信息：结论+适用条件+反例+来源链+置信度。
- 置信度三档：官方实证 > 多源交叉 > 孤证；逐卡明标。
- 宁可 20 张硬卡不要 200 张水卡；无可信来源时如实写"暂无可信来源"。
- 消费出口：门道库（按问题组织）+ chat 按身份注入；**不做信息流**。
