...
## 追加（09-06）：推送中文化 + 配额保底
- [x] 用户反馈"推送全英文看不懂"→ 五类消息全部改中文，正文三段式（发生了什么/这意味着什么/需要做什么），原始日志只作【证据】段保留
- [x] 配额保底：INFO（日常封禁通知，最频繁）每日最多推 3 条，给警告/严重留 2 个位，防噪声挤掉真告警
- [x] 检查逻辑/冷却/状态文件/心跳零改动；alerts.log 级别 token 保持英文便于 grep
